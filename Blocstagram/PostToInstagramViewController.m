...
//  Blocstagram
//
//  Created by Matt Krueger on 7/17/15.
...
- (void)viewWillLayoutSubviews {
    ...
    [self.view addConstraints:@[imageHeightConstraint, imageWidthConstraint, centerImageConstraint, previewImageViewTopConstraint]];
    ...
    if (CGRectGetHeight(self.view.frame) > 500) {
        ...
        NSLayoutConstraint *sendButtonCenterConstraint = [NSLayoutConstraint constraintWithItem:self.sendButton attribute:NSLayoutAttributeCenterX relatedBy:0 toItem:self.view attribute:NSLayoutAttributeCenterX multiplier:1 constant:0];
        ...
        [self.view addConstraints:@[sendButtonCenterConstraint, sendButtonHeightConstraint, filterCollectionHeightConstraint]];
        ...
        NSArray *verticalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:[_previewImageView]-20-[_filterCollectionView]|" options:0 metrics:nil views:viewsDictionary];
...
- (void) sendImageToInstagramWithCaption:(NSString *)caption {
    ...
    NSURL *fileURL = [[tmpDirURL URLByAppendingPathComponent:@"blocstagram"] URLByAppendingPathComponent:@"igo"];
    ...
    if (!success) {
        ...
        [alertVC addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"OK", @"OK button") style:UIAlertControllerStyleAlert handler:nil]];
        [self presentViewController:alertVC animated:YES completion:nil];
        return;
    }
...
- (void) addfiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *pixelFilter = [CIFilter filterWithName:@"CIPizellate"];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        CIFilter *whiteSpecks = [CIFilter filterWithName:@"CIColorMatrix" keysAndValues:kCIInputImageKey, randomImage, @"inputRVector", [CIVector vectorWithX:0.0 Y:1.0 Z:0.0 W:0.0],
                                 @"inputGVector", [CIVector vectorWithX:0.0 Y:1.0 Z:0.0 W:0.0],
                                 @"inputBVector", [CIVector vectorWithX:0.0 Y:1.0 Z:0.0 W:0.0],
                                 @"inputAVector", [CIVector vectorWithX:0.0 Y:0.1 Z:0.0 W:0.0],
